  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg flex flex-col h-full min-h-[600px] shadow-sm">
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        {!hasOrders ? (
                          ...
                          <span className="bg-emerald-50 text-emerald-700 text-[10px] font-bold px-2 py-0.5 rounded uppercase border border-emerald-200">Order Placed</span>
                        ) : hasCancelled ? (
      ...
      {editPhoneModal.isOpen && (
        <div 
          className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200 overflow-hidden"
          onClick={() => setEditPhoneModal({ isOpen: false, group: null, phone: '', isSaving: false })}
        >
          <div 
            className="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-md flex flex-col max-h-[90vh] overflow-hidden"
          ...
          >
            ...
            <form onSubmit={handleSaveRecipientPhone} className="flex flex-col flex-1 min-h-0 overflow-hidden">
              ...
              <div className="flex items-center justify-end gap-2.5 p-4 sm:p-5 border-t border-slate-100 shrink-0 bg-slate-50">
                <button
                  type="button"
                  onClick={() => setEditPhoneModal({ isOpen: false, group: null, phone: '', isSaving: false })}
      ...
      {editMessageModal.isOpen && (
        ...
        >
          <div 
            className="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-lg flex flex-col max-h-[90vh] overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 shrink-0">
              <div>
            ...
            <form onSubmit={handleSaveMessage} className="flex flex-col flex-1 min-h-0 overflow-hidden">
              <div className="p-5 sm:p-6 space-y-4 overflow-y-auto custom-scrollbar flex-1">
                <div>
